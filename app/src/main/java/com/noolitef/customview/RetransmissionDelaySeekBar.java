...
public class RetransmissionDelaySeekBar extends AppCompatSeekBar {

    private int thumbSize;
    private TextPaint textPaint;
    private Rect bounds;

    public RetransmissionDelaySeekBar(Context context) {
        super(context);
        init();
    }

    public RetransmissionDelaySeekBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public RetransmissionDelaySeekBar(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init();
    }

    private void init() {
        thumbSize = getResources().getDimensionPixelSize(R.dimen.dp_68);
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        String progressText = Integer.toString(getProgress()).concat("0 мс");
